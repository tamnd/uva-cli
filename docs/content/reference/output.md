...
describes what users get. Pick a format with `-o`, or let uva choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
uva <command> -o table   # aligned columns for reading
uva <command> -o jsonl   # one JSON object per line, for piping
uva <command> -o json    # a single JSON array
uva <command> -o csv     # spreadsheet friendly
uva <command> -o tsv     # tab-separated
uva <command> -o url     # just the URL column
uva <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
uva <command> --fields id,title,url
```
...
```bash
uva <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
uva <command>            # a table, because this is a terminal
uva <command> | wc -l    # JSONL, because this is a pipe
```
